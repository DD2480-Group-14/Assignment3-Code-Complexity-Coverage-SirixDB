...
public final class JsonMultipleUpdatesTest extends AbstractJsonTest {
  ...
  /**
   * Test two replace operations WITHOUT let binding (multiple jn:doc calls).
   * This is the pattern that was originally generated by the web GUI.
   * Each jn:doc() call should share the same write transaction.
   */
  @Test
  ...
  /**
   * Test three replace operations WITHOUT let binding.
   * All three jn:doc() calls should share the same transaction.
   */
  @Test
  ...
  /**
   * Test replace + delete WITHOUT let binding.
   * Both jn:doc() calls should share the same transaction.
   */
  @Test
  ...
  /**
   * Test multiple sdb:select-item calls WITHOUT let binding.
   * Each sdb:select-item uses a fresh jn:doc() call.
   */
  @Test
  ...
  /**
   * Test path-based replace + delete via sdb:select-item with let binding.
   * This reproduces the exact web GUI commit query pattern:
   * - Updates use path navigation: $doc.path.to.field
   * - Deletes use sdb:select-item: sdb:select-item($doc, nodeId)
   * 
   * IMPORTANT: When deleting an object property, you must select the OBJECT_RECORD node
   * (the key-value pair), not the VALUE node. Use sdb:nodekey() on $doc.property to get it.
   */
  @Test
  ...
  /**
   * Test multiple path-based replaces + single select-item delete.
   * This uses sdb:nodekey() to get the correct OBJECT_RECORD node key.
   */
  @Test
   ...
   * Therefore, the GUI should use PATH-BASED deletion ($doc.path.to.property)
   * instead of sdb:select-item with node keys for object properties.
   */
  @Test
  ...
  /**
   * Test delete array item using sdb:select-item.
   * Array items CAN be deleted by node key (unlike object record values).
   */
  @Test
  ...
  /**
   * Test nested structure with array access + path-based delete.
   * Simulates the chicago-v2/products scenario from the GUI.
   */
  @Test
